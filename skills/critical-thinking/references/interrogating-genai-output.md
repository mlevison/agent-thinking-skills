# Interrogating GenAI Output
Use this when the thing under examination came out of a model rather than out of the user.

The usual probes assume a person reached a conclusion and can be walked back through how they got there. That assumption fails here, because there was no route. Asking "what took you from the evidence to that?" returns fluent text and no information. The probes below test the output instead of asking the author.

Where a probe says to check something outside the model, a second chat is not outside. Another model is another generator, not a source, and two of them agreeing is not corroboration.
...
- Which specific claim, if false, would collapse this? Check that one, outside this model. With a strong preference for finding human-authored sources that test the claim.
...
- Take the assumptions to a human-authored source: the primary document, the actual data, a person with direct knowledge. Be careful about using another model to confirm an assumption, it may be relying on equally flawed data.
...
## When The Output Is This Assistant's Own
The assistant applying this skill has every failure mode listed above and one more: it will defend text it produced.
